...
def handle_message(event):
    ...
    if not found_shops:
        reply_msgs.append(TextSendMessage(text="⚠️ 請輸入有效的店名，例如: 國順 義和"))
    else:
        # 各別輸出每家
        for name, url, _, _ in found_shops:
            reply_msgs.append(TextSendMessage(text=f"{name} 導航：{url}"))
        ...
        if "導航" in text and len(found_shops) >= 2:
            ...
            # ⚠️ 拿掉 origin，Google Maps 會自動抓手機目前位置
            maps_url = f"{base_url}&destination={destination}&waypoints={waypoints}"
            reply_msgs.append(TextSendMessage(text=f"🗺️ 導航路線：\n{maps_url}"))
